...
def duplicate_inplace(x: torch.Tensor, dup = 8, interleave = False):
    """ Input x should be of shape (resolution, N, dim) """
    if interleave:
        resolution, _, channel = x.shape[-1]
...
def eval_integrand(samples: torch.Tensor):
...
def generate_training_samples(idx_r, g = 0.7, resolution = 256, sp_per_dim = 8):
    ...
    """
    div_r = (0.999 - 0.2) / resolution
    div_a = PI_2 / resolution
    alphas = torch.arange(resolution, *TENSOR_PROP) * div_a
    r_s   = 0.2 + div_r * idx_r
